...
const Admin_UploadReport = () => {
  ...
  const occupations = ["Parents", "Alumni", "Guest Speaker", "Coach", "Visitors", "Others"];
  ...
  const handleChange = (e) =>
    ...
    const handleImageUpload = (e) => {
      ...
      if (file.type === "image/png" || file.type === "image/jpeg") {
        ...
        reader.onloadend = () => setImage(reader.result); // preview
 ...
 const handleSubmit = async () => {
  if (!formData.itemName || !formData.lostFound || !formData.firstName || !formData.lastName || !formData.email) {
  ...
  try {
    ...
    const res = await fetch("http://localhost:5000/api/admin-upload/external/submit", {
      method: "POST",
      body: fd,
    });
    ...
    try {
      ...
      if (!res.ok) throw new Error(text || "Server returned an invalid response");
    }
    if (!res.ok) throw json || { error: "Failed to submit report" };
    ...
    console.error(err);
    alert(err?.error || err?.message || "Failed to submit report");
  }
};
  ...
  <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4 sm:p-6">
    ...
    <div className="w-full max-w-3xl flex justify-start mb-6">
              ...
              <label className="md:col-span-2 relative">
                ...
                <div className="relative">
                  ...
                  <Calendar
                    ...
                    onClick={() =>
                      document
                        .querySelector('input[name="dateReported"]')
                        .showPicker?.()
                    }
                  />
                </div>
      ...
      {step === 2 && (
        <div className="bg-[#DCD6F7] rounded-2xl shadow-2xl p-6 sm:p-8 w-full max-w-3xl relative">
          <h2 className="text-2xl font-bold text-[#3A0CA3] mb-6">
            Contact Information
          </h2>

          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 relative z-10 text-[#3A0CA3]">
            <label>
              First Name
              <input
                type="text"
                ...
                onChange={handleChange}
                className="h-[35px] border p-2 rounded-full w-full bg-white focus:ring-2 focus:ring-purple-500 outline-none"
              />
            </label>
            <label>
              Middle Name
            ...
            <label>
              ...
              <input
                type="email"
                name="email"
                placeholder="e.g. juancruz@email.com"
...
export default Admin_UploadReport;